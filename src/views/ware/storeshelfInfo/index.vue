<template>
  <div>
    <ProTable
      ...
      :columns="columns"
      :requestApi="getStoreshelfInfoList"
    >
...
import type { WarehouseInfo, StoreshelfInfo } from '@/api/ware/types'
...
const columns: ColumnProps[] = [
  ...
  { prop: 'storehouseCount', label: '库位个数', width: 120 },
  {
    prop: 'status',
    label: '状态',
    render: ({ row }) => {
...
const initList = reactive({
  ...
  temperatureTypeList: [] as Dict.DictCodeItem[],
})
...
const openDialog = async (
  title: string,
  rowData: Partial<StoreshelfInfo.ResStoreshelfInfoItem> = {},
) => {
  const isAuth =
    title === '新增'
  ...
  const params = {
    ...
    temperatureTypeList: initList.temperatureTypeList,
    api: title === '新增' ? addStoreshelfInfo : updateStoreshelfInfo,
...
// *根据id删除用户
const handleDelete = async (row: StoreshelfInfo.ResStoreshelfInfoItem) => {
  await useAuth(hasAuth('btn.shipperInfo.remove'))
  await useHandleData(deleteStoreshelfInfoById, row.id, `删除${row.name}用户`)
...
const init = () => {
  ...
  findByDictCode('HouseType').then((res) => {
    initList.areaTypeList = res.data
  })
  findByDictCode('TemperatureType').then((res) => {
    initList.temperatureTypeList = res.data
  })
}
